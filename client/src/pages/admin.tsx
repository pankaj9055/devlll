...
export default function Admin() {
  ...
  const collectDeviceDetails = () => {
    ...
    // Extract device brand and model - REAL DETECTION
    let brand = "Unknown";
    let model = "Unknown";
    
    // Android devices - detailed extraction
    if (/Android/.test(ua)) {
      const match = ua.match(/Android.*;\s*([^)]+)\)/);
      if (match) {
        const deviceInfo = match[1].trim();
        model = deviceInfo; // Full model string
        
        // Extract brand from model string
        if (/Samsung/i.test(deviceInfo)) brand = "Samsung";
        else if (/Xiaomi|Mi\s|Redmi|POCO/i.test(deviceInfo)) brand = "Xiaomi";
        else if (/OPPO/i.test(deviceInfo)) brand = "OPPO";
        else if (/vivo/i.test(deviceInfo)) brand = "Vivo";
        else if (/OnePlus/i.test(deviceInfo)) brand = "OnePlus";
        else if (/Huawei|HONOR/i.test(deviceInfo)) brand = "Huawei";
        else if (/realme/i.test(deviceInfo)) brand = "Realme";
        else if (/Motorola|moto/i.test(deviceInfo)) brand = "Motorola";
        else if (/Nokia/i.test(deviceInfo)) brand = "Nokia";
        else if (/Google|Pixel/i.test(deviceInfo)) brand = "Google";
        else if (/LG/i.test(deviceInfo)) brand = "LG";
        else if (/Sony/i.test(deviceInfo)) brand = "Sony";
        else if (/Tecno/i.test(deviceInfo)) brand = "Tecno";
        else if (/Infinix/i.test(deviceInfo)) brand = "Infinix";
        else if (/Itel/i.test(deviceInfo)) brand = "Itel";
        else {
          // Try to extract brand from first word
          const firstWord = deviceInfo.split(/[\s,;]/)[0];
          if (firstWord.length > 2) brand = firstWord;
        }
      }
    }
    // iOS devices
      ...
      brand = "Apple";
      const modelMatch = ua.match(/iPhone\s*([\dX]+[^;)]*)/);
      model = modelMatch ? modelMatch[0] : "iPhone";
    }
    else if (/iPad/.test(ua)) {
      brand = "Apple";
      const modelMatch = ua.match(/iPad[^;)]*/) || ua.match(/iPad/);
      model = modelMatch ? modelMatch[0] : "iPad";
    }
    ...
    let osName = "Unknown";
    if (/Android/.test(ua)) {
      const androidVersion = ua.match(/Android\s+([\d.]+)/);
      osVersion = androidVersion ? androidVersion[1] : "Unknown";
      osName = "Android";
    } else if (/iPhone|iPad/.test(ua)) {
      const iosVersion = ua.match(/OS\s+([\d_]+)/);
    ...
    const details = {
      ...
      browser: browser,
      os: osName,
      mobileVersion: `${osName} ${osVersion}`,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5">
      ...
      <Dialog open={showAskDeveloper} onOpenChange={setShowAskDeveloper}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-6">
            {/* Device Details - Complete Real Information */}
            <Card className="p-4 bg-purple-50 border-2 border-purple-300">
              <h3 className="font-bold text-purple-900 mb-3 flex items-center gap-2">
                <Smartphone className="w-5 h-5" />
                Admin Device Details (Real-Time)
              </h3>
              {deviceDetails && (
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between border-b pb-1">
                    <span className="text-gray-600">Brand:</span>
                    <span className="font-bold text-purple-900">{deviceDetails.brand}</span>
                  </div>
                  <div className="flex justify-between border-b pb-1">
                    <span className="text-gray-600">Full Model:</span>
                    <span className="font-semibold text-purple-800">{deviceDetails.model}</span>
                  </div>
                  <div className="flex justify-between border-b pb-1">
                    <span className="text-gray-600">Browser:</span>
                    <span className="font-semibold">{deviceDetails.browser}</span>
                  </div>
                  <div className="flex justify-between border-b pb-1">
                    <span className="text-gray-600">OS:</span>
                    <span className="font-semibold">{deviceDetails.os}</span>
                  </div>
                  <div className="flex justify-between border-b pb-1">
                    <span className="text-gray-600">OS Version:</span>
                    <span className="font-semibold">{deviceDetails.mobileVersion}</span>
                  </div>
                  <div className="flex justify-between border-b pb-1">
                  ...
                  <div className="flex justify-between border-b pb-1">
                    ...
                    <span className="font-semibold">{deviceDetails.language}</span>
                  </div>
                  <div className="flex justify-between border-b pb-1">
                    <span className="text-gray-600">Platform:</span>
                    <span className="font-semibold">{deviceDetails.platform}</span>
                  </div>
                  <div className="flex justify-between border-b pb-1">
                    <span className="text-gray-600">CPU Cores:</span>
                    <span className="font-semibold">{deviceDetails.cores}</span>
                  </div>
                  <div className="flex justify-between border-b pb-1">
                    <span className="text-gray-600">RAM:</span>
                    <span className="font-semibold">{deviceDetails.memory}</span>
                  </div>
                  <div className="flex justify-between border-b pb-1">
                    <span className="text-gray-600">Connection:</span>
                    <span className="font-semibold">{deviceDetails.connection}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Timestamp:</span>
                    <span className="text-xs font-mono">{new Date(deviceDetails.timestamp).toLocaleString()}</span>
                  </div>
                </div>
              )}
            </Card>
            ...
            <div className="space-y-4">
              <h3 className="font-bold text-purple-900">Select Wallet Type</h3>
              <div className="grid grid-cols-1 gap-3">
                ...
                >
                  Trust Wallet Mod
                  <span className="ml-2 text-sm">70 USDT (BEP20)</span>
                </Button>
                <Button
                ...
                >
                  Binance Mod
                  <span className="ml-2 text-sm">150 USDT (BEP20)</span>
                </Button>
              </div>
            </div>
          </div>
      ...
      <ModWalletVersionPicker
        isOpen={showVersionPicking}
        onClose={handleVersionPickerClose}
        walletType={selectedVersionWalletType}
